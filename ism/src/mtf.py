...
class mtf:
    ...
    def mtfWfeAberrations(self, fr2D, lambd, kLF, wLF, kHF, wHF):
        ...
        """
        #TODO
        Hwfe = np.exp(-fr2D*(1-fr2D)*(kLF*math.pow(wLF/lambd, 2)+kHF*math.pow(wHF/lambd, 2)))
        return Hwfe
    ...
    def mtfSmearing(self, fnAlt, ncolumns, ksmear):
        ...
        sincv = np.vectorize(sincf)

        Hsmear_col = sincv(ksmear*fnAlt)
        Hsmear_col = Hsmear_col.repeat(ncolumns, -1)
        Hsmear = Hsmear_col.reshape(-1, ncolumns)
        return Hsmear
