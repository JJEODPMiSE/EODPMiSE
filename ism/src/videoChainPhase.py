...
class videoChainPhase(initIsm):
    ...
    def digitisation(self, toa, bit_depth, min_voltage, max_voltage):
        ...
        """
        #TODO
        toa_dn = np.round((toa/(max_voltage-min_voltage))*(pow(2,bit_depth)-1))
        index1 = np.where(toa_dn<0)
        toa_dn[index1] = 0
        index2 = np.where(toa_dn>(pow(2,bit_depth)-1))
        toa_dn[index2] = pow(2,bit_depth)-1
        return toa_dn
