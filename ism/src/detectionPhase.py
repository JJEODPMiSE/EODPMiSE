...
class detectionPhase(initIsm):
    ...
    def irrad2Phot(self, toa, area_pix, tint, wv):
        ...
        """
        #TODO
        Energy_in = toa/1000*area_pix*tint
        Engegy_photon = self.constants.h_planck*self.constants.speed_light/wv
        toa = Energy_in/Engegy_photon
        return toa
    ...
    def prnu(self, toa, kprnu):
        ...
        """
        #TODO
        PRNUact = np.random.normal(loc=0.0, scale=1.0, size=toa.shape[1])*kprnu
        for i in range(toa.shape[1]):
            toa_prnu = toa*(1+PRNUact)
        return toa_prnu
