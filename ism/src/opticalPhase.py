...
class opticalPhase(initIsm):
    ...
    def applySysMtf(self, toa, Hsys):
        ...
        :return: TOA image in irradiances [mW/m2], real part of the matrix
        """
        # TODO
        GE = fft2(toa)
    ...
    def spectralIntegration(self, sgm_toa, sgm_wv, band):
        ...
        for i in range(sgm_toa.shape[0]):
            for j in range(sgm_toa.shape[1]):
                cs = interp1d(sgm_wv/1000, sgm_toa[i, j, :], fill_value=(0, 0),
                              bounds_error=False)
                toa_interp = cs(isrf_wv)
                toa_integration = np.multiply(toa_interp, isrf)
                toa[i][j] = np.sum(toa_integration)
        return toa
